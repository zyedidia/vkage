...
static void *vduse_log_get(const char *filename, size_t size)
{
    ...
    fd = open(filename, O_RDWR | O_CREAT | O_CLOEXEC, 0600);
    if (fd == -1) {
        return MAP_FAILED;
    }
...
static int vduse_queue_check_inflights(VduseVirtq *vq)
{
    ...
    /*
     * Local change: the reconnect log is optional. Without one there is no
     * inflight state to recover, but the index fixup and the IRQ injection
...
{
    /* Local change: no reconnect log means no inflight tracking. */
    if (!vq->log) {
        return 0;
    }
                               ...
                               uint64_t addr, size_t len)
{
    ...
    while (len) {
        ...
        addr += read_len;
        /*
         * Local change: read_len is a byte count, so advancing a
         * struct vring_desc * by it overshoots by sizeof(struct vring_desc).
         * This loop only iterates more than once when the indirect table
         * straddles an IOVA region boundary, which is exactly when this
         * function is called, so the overshoot ran off the caller's
         * desc_buf[VIRTQUEUE_MAX_SIZE] stack array.
         */
